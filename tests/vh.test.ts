...
test('v and h', async () => {
  const result = await buildTestFile('vh.xml', {logging: true});
  console.log(result);
...
test('vh with leaf text node', async () => {
  ...
  // console.log(result);
  expect(result).toBe('<div style="display:flex;flex:1 1 0;flex-direction:column;"><div style="flex:1 1 0;"><div style="flex:1 1 0;">  h i </div></div><div style="flex:1 1 0;"> i </div></div>');
});
